...
function App() {
  ...
  useEffect(() => {
    async function getEngines() {
      try {
        ...
        await connect('mongodb+srv://shunitou:lcoVqBPKUwKrnlvo@cluster0.xnyrnbf.mongodb.net/?retryWrites=true&w=majority', { useNewUrlParser: true, useUnifiedTopology: true });
        console.log('Database connected');

        // Retrieve the models from the database
        const modelsData = await mongoose.connection.db.collection('models').find().toArray();
  ...
  async function handleSubmit(e) {
    ...
    try {
      // Store the chat logs in the database using mongoose
      await mongoose.connection.db.collection('chatLogs').insertOne({
        messages: messages,
        currentModel: currentModel
      });

      const response = await mongoose.connection.db.collection('response').insertOne({
        method: "POST",
